...
export default function AnaliseTemporalPage({ periodo, fonteAtiva }) {
  const [modo, setModo] = useState('evolucao');
  const [periodosSel, setPeriodosSel] = useState([null, null, null]);
  ...
  const aplicarAtalho = (tipo) => {
    ...
    const antepenultimo = sorted.length > 2 ? sorted[sorted.length - 3] : null;
  ...
  useEffect(() => {
    const mesesParaCarregar = periodosSel.filter((m) => m && !ufs[m]);
  ...
  const variacoes = useMemo(() => {
    const result = [];
    const campos = [
      { key: 'receita', label: 'Receita', fmt: formatCurrency },
      { key: 'pedidos', label: 'Pedidos', fmt: (v) => String(v || 0) },
      { key: 'ticket_medio', label: 'Ticket Medio', fmt: formatCurrency },
    ];

    const validos = periodosDados.filter(Boolean);
    ...
    return (
      <div className="flex min-h-[300px] items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Carregando dados temporais...</p>
        </div>
      </div>
    );
  }
        ...
        <>
          ...
          {periodosDados.filter(Boolean).length >= 2 && (
            <SectionCard title="Comparacao Visual">
              {['receita', 'pedidos', 'ticket_medio'].map((campo) => {
