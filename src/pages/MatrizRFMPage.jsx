...
function simulateRFM(clientes) {
  return clientes.map((c, i) => {
    const rScore = c.r_score || Math.max(1, Math.min(5, 5 - Math.floor(i / 2)));
    const fScore = c.f_score || Math.max(1, Math.min(5, Math.min(c.pedidos || 1, 5)));
    const mScore = c.m_score || Math.max(1, Math.min(5, Math.ceil((c.valor || 0) / 600)));
    return { ...c, r_score: rScore, f_score: fScore, m_score: mScore };
  });
}
...
export default function MatrizRFMPage({ periodo, fonteAtiva }) {
      ...
      try {
        ...
        if (!error && data && data.length > 0 && !cancelled) {
          setClientes(data.map((d) => ({
            nome: d.nome || d.cliente_nome || 'Sem nome',
            valor: d.valor_total || d.monetary || 0,
            pedidos: d.total_pedidos || d.frequency || 0,
            r_score: d.r_score || 3,
            f_score: d.f_score || 3,
            m_score: d.m_score || 3,
            tipo: d.tipo || 'B2C',
            uf: d.uf || '',
            ticketMedio: d.ticket_medio || 0,
            ultimaCompra: d.ultima_compra || null,
          })));
        } else if (!cancelled) {
          // Fallback
  ...
  function toggleSegmentExpand(key) {
      ...
      <SectionCard title="Acoes Priorizadas">
        <div className="space-y-3">
          {segmentosData
            ...
            .sort((a, b) => {
              const priority = { critico: 0, alto: 1, medio: 2, baixo: 3, positivo: 4 };
