...
export default function ClientesPage({ periodo, fonteAtiva }) {
  const [range, setRange] = useState({ dataIni: '', dataFim: '' });

  const topClientes = DADOS.topClientes || [];
  const base = DADOS.baseClientesReal || {};
  const porUF = base.porUF || [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <KPICard
          ...
          subvalue={`${periodo || 'Periodo atual'}`}
          color="green"
        />
        <KPICard
          label="Taxa Recompra"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-800">
                ...
                <p className="mt-1 text-xl font-bold text-blue-600 dark:text-blue-400">
                  {formatNumber(base.pessoaFisica || 7830)}
                </p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-800">
                ...
                <p className="mt-1 text-xl font-bold text-orange-600 dark:text-orange-400">
                  {formatNumber(base.pessoaJuridica || 166)}
                </p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-800">
                <p className="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Com Email</p>
                <p className="mt-1 text-xl font-bold text-green-600 dark:text-green-400">
                  {base.percentEmail || 76.6}%
                </p>
                <p className="text-xs text-gray-500">{formatNumber(base.comEmail || 6127)} contatos</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-800">
                <p className="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">Com Celular</p>
                <p className="mt-1 text-xl font-bold text-green-600 dark:text-green-400">
                  {base.percentCelular || 76.8}%
                </p>
                <p className="text-xs text-gray-500">{formatNumber(base.comCelular || 6148)} contatos</p>
              </div>
            </div>
          </div>
        </SectionCard>
